...
urlpatterns = [
    ...
    url(
        _(r"^route/(?P<pk>\d+)/$"),
        EditDeliveryOfToday.as_view(),
        name="edit_delivery_of_today",
    ),
    url(
        _(r"^route/(?P<pk>\d+)/create/$"),
        CreateDeliveryOfToday.as_view(),
        name="create_delivery_of_today",
    ),
    url(_(r"^kitchen_count/$"), KitchenCount.as_view(), name="kitchen_count"),
    url(
        _(r"^kitchen_count/(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d+)/$"),
        KitchenCount.as_view(),
        name="kitchen_count_date",
    ),
    url(
