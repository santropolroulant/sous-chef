...
1. Sous-Chef currently runs using Python 3.7. Here's how to compile it:

```
apt install build-essential zlib1g-dev libbz2-dev uuid-dev libssl-dev \
...
```
apt install mariadb-server nginx libnginx-mod-http-fancyindex gdal-bin \
...
6. Initialize Sous-Chef

```bash
cd /usr/local/lib/python3.7/site-packages/souschef
...
```bash
cp /usr/local/lib/python3.7/site-packages/souschef/configsamples/nginx.conf /etc/nginx/sites-available/souschef
...
8. Create the Sous-Chef service
...
```
cp /usr/local/lib/python3.7/site-packages/souschef/configsamples/souschef.service /lib/systemd/system/souschef.service
...
9. Setup the Sous-Chef cron job
